...
public class SumOperator extends Operator{
    ...
    public SumOperator(Operator operator, GroupByElement groupBy, SelectItem<?> sumExpression) {
        ...
        if(groupBy!=null && sumExpression!=null){
            //Group by with sum aggregate Eg: SELECT Sailors.A. SUM(5*Sailors.b) FROM Sailors GROUP BY Sailors.A
            while (tuple != null) {
                ...
                this.sumMap.put(groupByAttributeValue, this.sumMap.getOrDefault(groupByAttributeValue, 0) + sumAttributeValue);
                tuple=this.childOperator.getNextTuple();
            }
        } else if (groupBy!=null){
            // Eg: Group  by without sum aggregate. Eg: SELECT Sailors.A FROM Sailors GROUP BY Sailors.A
            while (tuple!= null) {
                List<Integer> groupByAttributeValue = new ArrayList<>();
                for(Integer attributeIndex: groupByAttributeIndexList){
                    groupByAttributeValue.add(tuple.getTupleField(attributeIndex));
    ...
    /**
     * Return the sum value for the tuple based on the sum expression to be added to the aggregate
    ...
    /**
     * Return the result of the sum aggregate with or without the presence of group by
     * @return result of the query after sum aggregate
     */
     public List<Tuple> getResults() {
         List<Tuple> results=new ArrayList<>();
         if(this.sumExpression!=null && this.groupBy!=null){ // Eg: SELECT Sailors.A. SUM(5*Sailors.b) FROM Sailors GROUP BY Sailors.A
         ...
         else if(this.sumExpression!=null && this.sumResultWithoutGroupBy!=null){ // Eg: SELECT SUM(1) FROM Sailors
