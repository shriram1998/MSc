...
public class ProjectOperator extends Operator{
    ...
    /**
     * ProjectOperator constructor is all tuples are present already. Eg: Projection after sum aggregate
     * @param allTuples all tuples of Child operator
     * @param selectItems list of project attributes
     */
    public ProjectOperator(List<Tuple> allTuples, List<SelectItem<?>> selectItems) {
        this.attributes=selectItems;
        ...
        for(SelectItem<?> attribute:this.attributes){
            if(!attribute.toString().contains("SUM")){
                selectedIndices.add(allTuples.get(0).getAttributeIndex(attribute.toString()));
     ...
     */
    public List<Tuple> getAllProjectedTuples(){ return this.allTuples;}
