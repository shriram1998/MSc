...
def _predict_tag_logreg(token, model, tag_encoder):
    ...
    if token.text=='rooms':
        print(token.text,sorted_distribution)
    return sorted_distribution

def _my_model_predict_tag_logreg_updated(token, model, tag_encoder):
    """Predict tag according to logistic regression on word embedding.
    ...
    Returns:
        (List(Tuple(str,float))): distribution over tags, sorted from most to
          least probable
    """
    embedding=token_embedding_with_additional_features(token)
    # embedding_normalized = sklearn.preprocessing.normalize(embedding.reshape(1,-1), norm='l1')
    
    log_probs = model.predict_log_proba([embedding])[0]
    distribution = list(zip(tag_encoder.classes_, log_probs))
    sorted_distribution = \
        sorted(distribution, key=lambda tag_logprob_pair: -tag_logprob_pair[1])
    if token.text=='rooms':
...
def int_to_one_hot(integer, num_classes=20):
    """
    ...
    print(integer,len(one_hot_encoding),num_classes)
    one_hot_encoding[integer] = 1
    return one_hot_encoding

def token_embedding_with_additional_features(token):
    token_pos_enc=int_to_one_hot(token.pos-84)
    token_head_pos_enc=int_to_one_hot(token.head.pos-84)
    # if token.ent_type==0:
    #     ent_enc=int_to_one_hot(18,19)
    # else:
    #     ent_enc=int_to_one_hot(token.ent_type-380,19)
    pos_embedding=np.concatenate((token_pos_enc,token_head_pos_enc))
    # pos_embedding_normalized = sklearn.preprocessing.normalize(pos_embedding.reshape(1,-1), norm='l2')
    embedding=np.concatenate((token.vector,token.head.vector))
    return embedding
...
def train_my_model(data):
    ...
    #
    ##########################################################################
    train_X = [token_embedding_with_additional_features(token) for
               sample in data for
               token in sample['annotated_text']]
    train_y = [token._.bio_slot_label for
               sample in data for
               token in sample['annotated_text']]
    # train_X_normalized = sklearn.preprocessing.normalize(train_X, norm='l2')
    ...
    print(f'> Finished training logistic regression on {len(train_X)} tokens')
    return lambda token: _my_model_predict_tag_logreg_updated(token, model, tag_encoder)
