...
def encode_token_freq(data, vocab):
    ...
    for exp in data:
        ...
        exp_vector = np.array([float(exp.split().count(word)) for word in vocab])
...
def feature_probabilities(training_x, training_y, alpha):
    ...
    for c in set(training_y):       #for each category
        ...
        # c_data stores feature values for the whole category c
        # i.e. feature values summed over all documents in category c
        c_data = training_x[c_exps, :].sum(axis=0, keepdims=True)
        c_data += alpha
...
def nb_posterior_log_probability(exp, c, c_probs, f_probs):
    ...
    """
    return c_probs[c] + np.multiply(f_probs[c], exp).sum()
